...
move1R = () => {
    ...
    if (this.maxVisibleSlides > 1) {
        phantom.style.left = (this.slideWidth + this.carddist / (this.maxVisibleSlides - 1)) * (this.slides.length) + "px"
    } else {
        phantom.style.left = (this.slideWidth + this.fixedCardDist) * (this.slides.length) + "px"
    }
    slidecont.appendChild(element)

    /*setTimeout(() => {
        if (this.maxVisibleSlides > 1) {
            phantom.style.left = (this.slideWidth + this.carddist / (this.maxVisibleSlides - 1)) * (1) + "px"
        } else {
            phantom.style.left = (this.slideWidth + this.fixedCardDist) * (1) + "px"
        }
    }, 10); phantom.style.left = "0px"(?) */


    this.slides.forEach((e, index) => {
        setTimeout(() => {
            if (this.maxVisibleSlides > 1) {
                e.style.left = (this.slideWidth + this.carddist / (this.maxVisibleSlides - 1)) * (index - 1) + "px"
            } else {
